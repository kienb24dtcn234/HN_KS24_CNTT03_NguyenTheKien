...
class OrderServiceImplTest {
    ...
    @Test
    void createOrder_shouldSavePending_computeAmount_andPublishEvent() {
        when(inventoryClientService.getProduct(1L)).thenReturn(
                ProductDto.builder().id(1L).name("iPhone 15 Pro").price(new BigDecimal("25000000")).stock(50).build());
        ...
        assertThat(result.getTotalAmount()).isEqualByComparingTo("50000000"); // 25tr * 2
        verify(orderEventProducer).publish(any()); // đã phát ORDER_CREATED khởi động Saga
    }
